...
export default class WorkItemsManagement extends Vue {
    ...
    get pendingItem(): WorkItemDto | null {
        return store.workItem.getters(store.workItem.getter.PendingWorkItem);
    }

    get editedItemMeta(): WorkItemDto | null {
        return store.workItem.getters(store.workItem.getter.EditedWorkItemMeta);
    }

    get editedItem(): WorkItem | null {
        return store.workItem.getters(store.workItem.getter.EditedWorkItem);
    }
    ...
    public async confirmCreate(): Promise<void> {
        const id = await store.workItem.dispatch(store.workItem.action.CreateWorkItem);
    ...
    public async onFocusSessionStart(option: FocusSessionStartupOption): Promise<void> {
        ...
        if (await store.timeSession.dispatch(store.timeSession.action.StartFocusSession, option)) {
            await this.loadWorkItems();
        }
    }
    ...
    public async onBreakSessionEnd(id: string): Promise<void> {
        ...
        const isStopped = await store.timeSession.dispatch(store.timeSession.action.StopBreakSession, id);
    ...
    public onRatingsUpdate(): void {
        store.user.dispatch(store.user.action.UpdateUserRatings, this.ratingsChangeOption!.current);
        this.ratingsChangeOption = null;
    }

    public async onItemMetaUpdate(item: WorkItemDto): Promise<void> {
        if (await store.workItem.dispatch(store.workItem.action.UpdateWorkItemMeta, item)) {
    ...
    public async onItemUpdate(item: WorkItem): Promise<void> {
        if (await store.workItem.dispatch(store.workItem.action.UpdateWorkItem, item)) {
            this.$emit('item:update');
        }
    }

    public async onItemDelete(id: string): Promise<void> {
        if (await store.workItem.dispatch(store.workItem.action.DeleteWorkItem, id)) {
            this.$emit('item:delete');
        }
    }

    public async onItemSelect(id: string): Promise<void> {
        await store.workItem.dispatch(store.workItem.action.LoadEditedWorkItem, id);
    }

    public async onItemStart(item: WorkItemDto): Promise<void> {
        if (!store.timeSession.getters(store.timeSession.getter.HasActiveFocusSession)) {
            this.focusSessionOption = new FocusSessionStartDialogOption(item);
        }
        else if (await store.timeSession.dispatch(store.timeSession.action.SwitchWorkItem, item.id)) {
            this.$emit('item:update');
        }
    }

    public async onItemStop(targetStatus: WorkItemStatus): Promise<void> {
        if (await store.timeSession.dispatch(store.timeSession.action.StartOverlearning, targetStatus)) {
            this.$emit('item:update');
        }
    }

    public async loadWorkItems(): Promise<void> {
        await store.workItem.dispatch(store.workItem.action.LoadWorkItems, this.query);
    }

    private async showRatingsChange(): Promise<void> {
        const user = store.user.getters(store.user.getter.Profile);
        const ratings = await store.performance.dispatch(store.performance.action.GetPerformanceRating);
