...
describe('work item tracking stats group unit test', () => {
    ...
    beforeEach(() => {
        store = createStore();
        component = shallowMount(WorkItemTrackingStatsGroup, { global: { mocks: { $store: store.store } } });
    });
    ...
    describe('timeTracked', () => {
        test('should return total time tracked', () => {
            ...
            store.performance.commit(store.performance.mutation.SetActivityBreakdown, breakdown);
    ...
    describe('inaccurateEstimate', () => {
        test('should return inaccurate estimations', () => {
            ...
            store.performance.commit(store.performance.mutation.SetEstimationBreakdown, breakdown);
