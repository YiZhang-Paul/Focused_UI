...
describe('work items management unit test', () => {
    ...
    describe('onFocusSessionEnd', () => {
        beforeEach(() => {
            store.user.commit(store.user.mutation.SetProfile, new UserProfile());
        });
    ...
    describe('onItemClose', () => {
        test('should close edited item', () => {
            ...
            store.workItem.commit(store.workItem.mutation.SetWorkItems, [meta]);
            store.workItem.commit(store.workItem.mutation.SetEditedWorkItem, item);
