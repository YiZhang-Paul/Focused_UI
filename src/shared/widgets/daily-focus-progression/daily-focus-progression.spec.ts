...
describe('daily focus progression unit test', () => {
    ...
    describe('hoursFocused', () => {
        test('should return 0 when no hours focused', () => {
            ...
            store.performance.commit(store.performance.mutation.SetCurrentDayProgression, progress);
    ...
    describe('percentageStyle', () => {
        test('should return correct percentage style', () => {
            const mutation = store.performance.mutation.SetCurrentDayProgression;

            store.performance.commit(mutation, { ...new ProgressionCounter<number>(), current: 0 });
            expect(component.vm.percentageStyle.color).toEqual('var(--context-colors-alert-00)');

            store.performance.commit(mutation, { ...new ProgressionCounter<number>(), current: 5 });
            expect(component.vm.percentageStyle.color).toEqual('var(--context-colors-alert-00)');

            store.performance.commit(mutation, { ...new ProgressionCounter<number>(), current: 6 });
            expect(component.vm.percentageStyle.color).toEqual('var(--context-colors-regular-00)');

            store.performance.commit(mutation, { ...new ProgressionCounter<number>(), current: 11 });
            expect(component.vm.percentageStyle.color).toEqual('var(--context-colors-regular-00)');

            store.performance.commit(mutation, { ...new ProgressionCounter<number>(), current: 12 });
            expect(component.vm.percentageStyle.color).toEqual('var(--context-colors-warning-00)');

            store.performance.commit(mutation, { ...new ProgressionCounter<number>(), current: 18 });
            expect(component.vm.percentageStyle.color).toEqual('var(--context-colors-warning-00)');
        });
    });
});
