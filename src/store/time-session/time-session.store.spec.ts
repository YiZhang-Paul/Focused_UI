...
describe('time session store unit test', () => {
    ...
    describe('timeSessionStatus', () => {
        test('should return correct status when no session is active', () => {
            ...
            expect(store.timeSession.getters(store.timeSession.getter.TimeSessionStatus)).toEqual(TimeSessionStatus.Idle);
        });

        test('should return correct status during break session', () => {
            store.timeSession.commit(store.timeSession.mutation.SetActiveFocusSession, null);
            store.timeSession.commit(store.timeSession.mutation.SetActiveBreakSession, { ...new BreakSession(), id: '1' });

            expect(store.timeSession.getters(store.timeSession.getter.TimeSessionStatus)).toEqual(TimeSessionStatus.Resting);
        });
        ...
        test('should return correct status during focus session', () => {
            ...
            expect(store.timeSession.getters(store.timeSession.getter.TimeSessionStatus)).toEqual(TimeSessionStatus.Ongoing);

            session.workItems = [{ ...new WorkItemDto(), status: WorkItemStatus.Completed }];
            expect(store.timeSession.getters(store.timeSession.getter.TimeSessionStatus)).toEqual(TimeSessionStatus.Pending);
        });
    });
    ...
    describe('ongoingTimeSessionEnd', () => {
        test('should return correct session end for idle state', () => {
            ...
            expect(store.timeSession.getters(store.timeSession.getter.OngoingTimeSessionEnd)).toBeNull();
        });
        ...
        test('should return correct session end for break session', () => {
            ...
            expect(store.timeSession.getters(store.timeSession.getter.OngoingTimeSessionEnd)).toEqual(new Date(2021, 2, 1, 7, 45));
        });
        ...
        test('should return correct session end for focus session', () => {
            ...
            expect(store.timeSession.getters(store.timeSession.getter.OngoingTimeSessionEnd)).toEqual(new Date(2022, 5, 2, 7, 45));
        });
    });
    ...
    describe('startFocusSession', () => {
        test('should return false when failed to start focus session', async() => {
            ...
            const result = await store.timeSession.dispatch(store.timeSession.action.StartFocusSession, option);
    ...
    describe('stopFocusSession', () => {
        test('should return false when failed to stop focus session', async() => {
            ...
            const result = await store.timeSession.dispatch(store.timeSession.action.StopFocusSession, '1234');
    ...
    describe('startBreakSession', () => {
        test('should return false when failed to start break session', async() => {
            ...
            const result = await store.timeSession.dispatch(store.timeSession.action.StartBreakSession, option);
    ...
    describe('stopBreakSession', () => {
        test('should return false when failed to stop break session', async() => {
            ...
            const result = await store.timeSession.dispatch(store.timeSession.action.StopBreakSession, '1234');
    ...
    describe('loadActiveTimeSession', () => {
        test('should load active time sessions', async() => {
            ...
            expect(store.timeSession.getters(store.timeSession.getter.ActiveFocusSession)).not.toEqual(focusSession);
            expect(store.timeSession.getters(store.timeSession.getter.ActiveBreakSession)).not.toEqual(breakSession);

            await store.timeSession.dispatch(store.timeSession.action.LoadActiveTimeSession);

            sinonExpect.calledOnce(timeSessionHttpStub.getActiveFocusSessionMeta);
            sinonExpect.calledOnce(timeSessionHttpStub.getActiveBreakSession);
            expect(store.timeSession.getters(store.timeSession.getter.ActiveFocusSession)).toEqual(focusSession);
            expect(store.timeSession.getters(store.timeSession.getter.ActiveBreakSession)).toEqual(breakSession);
        });
    });
    ...
    describe('loadStaleTimeSession', () => {
        test('should load stale time sessions', async() => {
            ...
            expect(store.timeSession.getters(store.timeSession.getter.StaleFocusSession)).not.toEqual(focusSession);
            expect(store.timeSession.getters(store.timeSession.getter.StaleBreakSession)).not.toEqual(breakSession);

            await store.timeSession.dispatch(store.timeSession.action.LoadStaleTimeSession);

            sinonExpect.calledOnce(timeSessionHttpStub.getStaleFocusSessionMeta);
            sinonExpect.calledOnce(timeSessionHttpStub.getStaleBreakSession);
            expect(store.timeSession.getters(store.timeSession.getter.StaleFocusSession)).toEqual(focusSession);
            expect(store.timeSession.getters(store.timeSession.getter.StaleBreakSession)).toEqual(breakSession);
        });
    });
    ...
    describe('syncActiveTimeSession', () => {
        ...
        test('should reload when stale session detected', async() => {
            ...
            await store.timeSession.dispatch(store.timeSession.action.SyncActiveTimeSession);
        ...
        test('should sync focus session time', () => {
            ...
            expect(store.timeSession.getters(store.timeSession.getter.ActiveFocusSession)).toEqual(session);
        });
        ...
        test('should sync break session time', () => {
            ...
            expect(store.timeSession.getters(store.timeSession.getter.ActiveBreakSession)).toEqual(session);
        });
    });
});
