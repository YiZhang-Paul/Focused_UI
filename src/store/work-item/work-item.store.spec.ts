...
describe('work item store unit test', () => {
    ...
    describe('editedWorkItemMeta', () => {
        test('should return null when no edited work item exists', () => {
            store.workItem.commit(store.workItem.mutation.SetEditedWorkItem, null);

            expect(store.workItem.getters(store.workItem.getter.EditedWorkItemMeta)).toBeNull();
        });
        ...
        test('should return null when edited work item does not exist in work items list', () => {
            ...
            store.workItem.commit(store.workItem.mutation.SetEditedWorkItem, { ...new WorkItem(), id: '1' });
            store.workItem.commit(store.workItem.mutation.SetWorkItems, items);
        ...
        test('should return edited work item meta', () => {
            ...
            store.workItem.commit(store.workItem.mutation.SetEditedWorkItem, { ...new WorkItem(), id: '2' });
            store.workItem.commit(store.workItem.mutation.SetWorkItems, items);

            expect(store.workItem.getters(store.workItem.getter.EditedWorkItemMeta)?.id).toEqual('2');
        });
    });
    ...
    describe('workItems', () => {
        test('should always return sorted work items', () => {
            ...
            store.workItem.commit(store.workItem.mutation.SetWorkItems, unsorted);

            expect(store.workItem.getters(store.workItem.getter.WorkItems)).toEqual(sorted);
        });
    });

    describe('createWorkItem', () => {
        test('should do nothing when no pending work item exists', async() => {
            store.workItem.commit(store.workItem.mutation.SetPendingWorkItem, null);

            const result = await store.workItem.dispatch(store.workItem.action.CreateWorkItem);
        ...
        test('should return null when failed to create work item', async() => {
            ...
            store.workItem.commit(store.workItem.mutation.SetPendingWorkItem, new WorkItemDto());
            expect(store.workItem.getters(store.workItem.getter.PendingWorkItem)).toBeTruthy();
        ...
        test('should return work item id and clear pending work item on creation success', async() => {
            ...
            sinonExpect.calledOnce(workItemHttpStub.createWorkItem);
            expect(store.workItem.getters(store.workItem.getter.PendingWorkItem)).toBeNull();
    ...
    describe('updateWorkItem', () => {
        test('should do nothing and return false when update failed', async() => {
            ...
            const result = await store.workItem.dispatch(store.workItem.action.UpdateWorkItem, new WorkItem());
        ...
        test('should return true on update success', async() => {
            ...
            store.workItem.commit(store.workItem.mutation.SetWorkItems, [{ ...new WorkItemDto(), id: toUpdate.id, name: 'previous_name' }]);
            store.workItem.commit(store.workItem.mutation.SetEditedWorkItem, edited);

            const result = await store.workItem.dispatch(store.workItem.action.UpdateWorkItem, toUpdate);

            sinonExpect.calledOnce(workItemHttpStub.updateWorkItem);
            sinonExpect.calledOnce(workItemHttpStub.getWorkItemMeta);
            expect(store.workItem.getters(store.workItem.getter.WorkItems)[0]).toEqual(updated);
            expect(store.workItem.getters(store.workItem.getter.EditedWorkItem)).toEqual(edited);
        ...
        test('should return true and reload work item content when applicable on update success', async() => {
            ...
            workItemHttpStub.getWorkItemMeta.resolves(updated);
            store.workItem.commit(store.workItem.mutation.SetWorkItems, [{ ...new WorkItemDto(), id: toUpdate.id, name: 'previous_name' }]);
            store.workItem.commit(store.workItem.mutation.SetEditedWorkItem, toUpdate);
            ...
            sinonExpect.calledOnce(workItemHttpStub.getWorkItemMeta);
            expect(store.workItem.getters(store.workItem.getter.WorkItems)[0]).toEqual(updated);
            expect(store.workItem.getters(store.workItem.getter.EditedWorkItem)).toEqual(toUpdate);
            expect(result).toBeTruthy();
        });
    });
    ...
    describe('deleteWorkItem', () => {
        beforeEach(() => {
            store.workItem.commit(store.workItem.mutation.SetWorkItems, [{ ...new WorkItemDto(), id: '1' }]);
            store.workItem.commit(store.workItem.mutation.SetEditedWorkItem, { ...new WorkItem(), id: '1' });
        });
        ...
        test('should do nothing and return false when delete failed', async() => {
            ...
            const result = await store.workItem.dispatch(store.workItem.action.DeleteWorkItem, '1');

            sinonExpect.calledOnce(workItemHttpStub.deleteWorkItem);
            expect(store.workItem.getters(store.workItem.getter.WorkItems).length).toEqual(1);
            expect(store.workItem.getters(store.workItem.getter.WorkItems)[0].id).toEqual('1');
            expect(store.workItem.getters(store.workItem.getter.EditedWorkItem)?.id).toEqual('1');
            expect(result).toBeFalsy();
        });
        ...
        test('should return true on delete success', async() => {
            ...
            sinonExpect.calledOnce(workItemHttpStub.deleteWorkItem);
            expect(store.workItem.getters(store.workItem.getter.WorkItems).length).toEqual(0);
            expect(store.workItem.getters(store.workItem.getter.EditedWorkItem)).not.toBeNull();
            expect(result).toBeTruthy();
        });
        ...
        test('should return true and remove work item content when applicable on delete success', async() => {
            ...
            sinonExpect.calledOnce(workItemHttpStub.deleteWorkItem);
            expect(store.workItem.getters(store.workItem.getter.WorkItems).length).toEqual(0);
            expect(store.workItem.getters(store.workItem.getter.EditedWorkItem)).toBeNull();
            expect(result).toBeTruthy();
        });
    });
    ...
    describe('updateWorkItemMeta', () => {
        beforeEach(() => {
            store.workItem.commit(store.workItem.mutation.SetWorkItems, [{ ...new WorkItemDto(), id: '1', name: 'previous_name' }]);
        });

        test('should return false when failed to update work item meta', async() => {
            workItemHttpStub.updateWorkItemMeta.resolves(null);

            const result = await store.workItem.dispatch(store.workItem.action.UpdateWorkItemMeta, new WorkItemDto());

            sinonExpect.calledOnce(workItemHttpStub.updateWorkItemMeta);
            expect(store.workItem.getters(store.workItem.getter.WorkItems)[0].name).toEqual('previous_name');
            expect(result).toBeFalsy();
        });
        ...
        test('should return true and reload work item content when updated successfully', async() => {
            ...
            sinonExpect.calledOnce(workItemHttpStub.updateWorkItemMeta);
            expect(store.workItem.getters(store.workItem.getter.WorkItems)[0].name).toEqual('updated_name');
            expect(result).toBeTruthy();
        });
    });
    ...
    describe('loadEditedWorkItem', () => {
        test('should load edited work item', async() => {
            ...
            expect(store.workItem.getters(store.workItem.getter.EditedWorkItem)).not.toEqual(item);

            await store.workItem.dispatch(store.workItem.action.LoadEditedWorkItem, '1');

            sinonExpect.calledOnce(workItemHttpStub.getWorkItem);
            expect(store.workItem.getters(store.workItem.getter.EditedWorkItem)).toEqual(item);
        });
    });

    describe('loadWorkItems', () => {
        test('should use default query when not provided', async() => {
            expect(store.workItem.state().lastQuery).toBeFalsy();

            await store.workItem.dispatch(store.workItem.action.LoadWorkItems, null);

            expect(store.workItem.state().lastQuery).toBeTruthy();
        });
        ...
        test('should load work items and cache most recent query', async() => {
            ...
            expect(store.workItem.state().lastQuery).not.toEqual(query);

            await store.workItem.dispatch(store.workItem.action.LoadWorkItems, query);

            sinonExpect.calledOnce(workItemHttpStub.getWorkItems);
            expect(store.workItem.state().lastQuery).toEqual(query);
            expect(store.workItem.getters(store.workItem.getter.WorkItems)).toEqual(items);
        });
    });
});
